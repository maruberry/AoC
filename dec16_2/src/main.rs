...
fn main() {
    ...
    let answer = cycle_through(map);
    
    println!("Final answer is {}", answer);
    
}

fn cycle_through (map: Vec<Vec<u8>>) -> usize {
	
	let mut final_answer: usize = 0;
	let wide = map[0].len();
	let len = map.len();
	
	let mut start_loc: Vec<usize> = vec![0,0];
	
	for i in 0..len {
		start_loc[0] = i;
		start_loc[1] = 0;
		let mut start_dir = Direct::Right;
		
		let mut final_energy = light_flow(map.clone(), new_map(map.clone()), start_dir.clone(), &start_loc);
		let mut answer = answer_count(final_energy.clone());
		...
		start_loc[0] = i;
		start_loc[1] = wide - 1;
		start_dir = Direct::Left;
		
		final_energy = light_flow(map.clone(), new_map(map.clone()), start_dir.clone(), &start_loc);
		answer = answer_count(final_energy.clone());
